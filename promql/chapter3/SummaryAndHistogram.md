...
<details>
    ...
    **Solution.** You should have created a variable called service.
    ...
    ```
    histogram_quantile(0.5, rate(service_request_duration_seconds{service=~"$service"}[10m]))
    histogram_quantile(0.9, rate(service_request_duration_seconds{service=~"$service"}[10m]))
    histogram_quantile(0.95, rate(service_request_duration_seconds{service=~"$service"}[10m]))
    ```
</details>